...
gboolean on_album_artist_focus_out_event(GtkWidget *widget, GdkEventFocus *event,
										 gpointer user_data)
{
...
gboolean on_year_focus_out_event(GtkWidget *widget, GdkEventFocus *event, gpointer user_data)
{
	const gchar *ctext = gtk_entry_get_text(GTK_ENTRY(widget));

	if (strlen(ctext) != 4 || (ctext[0] != '1' && ctext[0] != '2') || ctext[1] < '0' ||
		ctext[1] > '9' || ctext[2] < '0' || ctext[2] > '9' || ctext[3] < '0' || ctext[3] > '9') {
...
gboolean on_album_title_focus_out_event(GtkWidget *widget, GdkEventFocus *event, gpointer user_data)
{
	const gchar *ctext = gtk_entry_get_text(GTK_ENTRY(widget));
	gchar *text = malloc(sizeof(gchar) * (strlen(ctext) + 1));
	if (text == NULL)
...
// lnr
gboolean on_album_genre_focus_out_event(GtkWidget *widget, GdkEventFocus *event, gpointer user_data)
{
	const gchar *ctext = gtk_entry_get_text(GTK_ENTRY(widget));
...
void on_artist_edited(GtkCellRendererText *cell, gchar *path_string, gchar *new_text,
					  gpointer user_data)
{
...
// lnr
void on_genre_edited(GtkCellRendererText *cell, gchar *path_string, gchar *new_text,
					 gpointer user_data)
{
	GtkListStore *store = GTK_LIST_STORE(
		gtk_tree_view_get_model(GTK_TREE_VIEW(lookup_widget(win_main, "tracklist"))));
...
void on_hybrid_toggled(GtkToggleButton *togglebutton, gpointer user_data)
{
...
void on_mp3bitrate_value_changed(GtkRange *range, gpointer user_data)
{
	char bitrate[8];
	bool vbr;
...
gchar *format_opus_bitrate(GtkScale *scale, gdouble arg1, gpointer user_data)
{
	return g_strdup_printf(_("%dKbps"), int_to_bitrate((int)arg1, FALSE));
}

gchar *format_musepack_bitrate(GtkScale *scale, gdouble arg1, gpointer user_data)
{
	return g_strdup_printf(_("%dKbps"), int_to_musepack_bitrate((int)arg1));
}

void on_pick_disc_changed(GtkComboBox *combobox, gpointer user_data)
{
	cddb_disc_t *disc = g_list_nth_data(gbl_disc_matches, gtk_combo_box_get_active(combobox));
	update_tracklist(disc);
}

void on_preferences_clicked(GtkToolButton *toolbutton, gpointer user_data)
{
	win_prefs = create_prefs();
	gtk_widget_show(win_prefs);
}

void on_prefs_response(GtkDialog *dialog, gint response_id, gpointer user_data)
{
...
void on_prefs_show(GtkWidget *widget, gpointer user_data)
{
	set_widgets_from_prefs(global_prefs);
}

void on_press_f2(void)
{
	GtkWidget *treeView;
...
void on_lookup_clicked(GtkToolButton *toolbutton, gpointer user_data)
{
...
void on_rip_button_clicked(GtkButton *button, gpointer user_data)
{
	GtkListStore *store = GTK_LIST_STORE(
		gtk_tree_view_get_model(GTK_TREE_VIEW(lookup_widget(win_main, "tracklist"))));
	if (store == NULL) {
...
void on_rip_mp3_toggled(GtkToggleButton *togglebutton, gpointer user_data)
{
...
void on_rip_flac_toggled(GtkToggleButton *togglebutton, gpointer user_data)
{
...
void on_rip_ogg_toggled(GtkToggleButton *togglebutton, gpointer user_data)
{
...
void on_rip_opus_toggled(GtkToggleButton *togglebutton, gpointer user_data)
{
...
void on_rip_wavpack_toggled(GtkToggleButton *togglebutton, gpointer user_data)
{
...
void on_rip_monkey_toggled(GtkToggleButton *togglebutton, gpointer user_data)
{
...
void on_single_artist_toggled(GtkToggleButton *togglebutton, gpointer user_data)
{
	...
	GtkTreeViewColumn *col =
		gtk_tree_view_get_column(GTK_TREE_VIEW(tracklist), COL_TRACKARTIST); //lnr
	gtk_tree_view_column_set_visible(col, !gtk_toggle_button_get_active(togglebutton));
}

// lnr
void on_single_genre_toggled(GtkToggleButton *togglebutton, gpointer user_data)
{
...
gboolean on_tracklist_mouse_click(GtkWidget *treeView, GdkEventButton *event, gpointer user_data)
{
