...
int main(int argc, char *argv[])
{
	...
	gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(tracklist), -1, _("Rip"), renderer,
	...
	gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(tracklist), -1, _("Artist"), renderer,
												"text", COL_TRACKARTIST, NULL);

	renderer = gtk_cell_renderer_text_new();
	g_object_set(renderer, "editable", TRUE, NULL);
	g_signal_connect(renderer, "edited", (GCallback)on_title_edited, NULL);
	gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(tracklist), -1, _("Title"), renderer,
												"text", COL_TRACKTITLE, NULL);

	renderer = gtk_cell_renderer_text_new();
	g_object_set(renderer, "editable", TRUE, NULL);
	g_signal_connect(renderer, "edited", (GCallback)on_genre_edited, NULL);
	gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW(tracklist), -1, _("Genre"), renderer,
												"text", COL_GENRE, NULL);
	...
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(pick_disc), renderer, "text", 0, NULL);
	renderer = gtk_cell_renderer_text_new();
	gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(pick_disc), renderer, TRUE);
	gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(pick_disc), renderer, "text", 1, NULL);
...
GtkTreeModel *create_model_from_disc(cddb_disc_t *disc)
{
	...
	store = gtk_list_store_new(NUM_COLS, G_TYPE_BOOLEAN, /* rip? checkbox */
	...
	for (track = cddb_disc_get_track_first(disc); track != NULL;
		...
		gtk_list_store_set(store, &iter, COL_RIPTRACK, track_format[cddb_track_get_number(track)],
						   ...
						   COL_GENRE, cddb_disc_get_genre(disc), -1);
	}
...
gpointer cddb_query_thread_run(gpointer data)
{
	...
	gbl_cddb_query_thread_num_matches =
		cddb_query(gbl_cddb_query_thread_conn, gbl_cddb_query_thread_disc);
...
cddb_disc_t *read_disc(char *cdrom)
{
		...
		if (ioctl(fd, CDIOREADTOCHEADER, &th) == 0) {
			...
			for (i = th.starting_track; i <= th.ending_track; i++) {
				...
				if (ioctl(fd, CDIOREADTOCENTRY, &te) == 0) {
					...
					cddb_track_set_frame_offset(track,
												ntohl(te.entry.addr.lba) + SECONDS_TO_FRAMES(2));
			...
			te.track = 0xAA;
			if (ioctl(fd, CDIOREADTOCENTRY, &te) == 0) {
				cddb_disc_set_length(disc, (ntohl(te.entry.addr.lba) + SECONDS_TO_FRAMES(2)) /
											   SECONDS_TO_FRAMES(1));
			}
		}
	}
	...
	if (status >= 0) {
		...
		if (ioctl(fd, CDIOREADTOCHEADER, &th) == 0) {
			...
			if (ioctl(fd, CDIOREADTOCENTRIES, &te) == 0) {
				cddb_disc_set_length(disc, (te.data->addr.lba + SECONDS_TO_FRAMES(2)) /
											   SECONDS_TO_FRAMES(1));
			}
		}
	}
...
void update_tracklist(cddb_disc_t *disc)
{
	...
	if (disc_artist != NULL) {
		...
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_main, "single_artist")),
									 singleartist);
	}
	if (disc_title != NULL) {
        ...
        */
		...
		if (g_list_length(gbl_disc_matches) > 1) {
			...
			for (curr = g_list_first(gbl_disc_matches); curr != NULL; curr = g_list_next(curr)) {
				...
				gtk_list_store_append(store, &iter);
				gtk_list_store_set(store, &iter, 0, cddb_disc_get_artist(tempdisc), 1,
								   cddb_disc_get_title(tempdisc), -1);
			}
