...
struct prefs *get_default_prefs(void)
{
	...
	if (p->cdrom == NULL || p->music_dir == NULL || p->format_music == NULL ||
		p->format_playlist == NULL || p->format_albumdir == NULL || p->server_name == NULL ||
		p->cddb_server_name == NULL) {
...
void set_widgets_from_prefs(struct prefs *p)
{
	...
	gtk_file_chooser_set_current_folder(GTK_FILE_CHOOSER(lookup_widget(win_prefs, "music_dir")),
										prefs_get_music_dir(p));
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "make_playlist")),
								 p->make_playlist);
	...
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_wav")),
								 p->rip_wav);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "do_fast_rip")),
								 p->do_fast_rip);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_mp3")),
								 p->rip_mp3);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_ogg")),
								 p->rip_ogg);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_flac")),
								 p->rip_flac);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_wavpack")),
								 p->rip_wavpack);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "mp3_vbr")),
								 p->mp3_vbr);
	...
	gtk_range_set_value(GTK_RANGE(lookup_widget(win_prefs, "flac_compression")),
						p->flac_compression);
	gtk_range_set_value(GTK_RANGE(lookup_widget(win_prefs, "wavpack_compression")),
						p->wavpack_compression);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "wavpack_hybrid")),
								 p->wavpack_hybrid);
	gtk_range_set_value(GTK_RANGE(lookup_widget(win_prefs, "wavpack_bitrate")), p->wavpack_bitrate);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_monkey")),
								 p->rip_monkey);
	gtk_range_set_value(GTK_RANGE(lookup_widget(win_prefs, "monkey_compression")),
						p->monkey_compression);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_aac")),
								 p->rip_aac);
	gtk_range_set_value(GTK_RANGE(lookup_widget(win_prefs, "aac_quality")), p->aac_quality);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_musepack")),
								 p->rip_musepack);
	gtk_range_set_value(GTK_RANGE(lookup_widget(win_prefs, "musepack_bitrate")),
						p->musepack_bitrate);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_opus")),
								 p->rip_opus);
	gtk_range_set_value(GTK_RANGE(lookup_widget(win_prefs, "opus_bitrate")), p->opus_bitrate);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "eject_on_done")),
								 p->eject_on_done);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "do_cddb_updates")),
								 p->do_cddb_updates);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "use_proxy")),
								 p->use_proxy);
	...
	gtk_entry_set_text(GTK_ENTRY(lookup_widget(win_prefs, "cddb_server_name")),
					   p->cddb_server_name);
	...
	if (global_prefs->more_formats_expanded) {
		gtk_expander_set_expanded(GTK_EXPANDER(lookup_widget(win_prefs, "more_formats_expander")),
								  TRUE);
	}
	if (global_prefs->proprietary_formats_expanded) {
		gtk_expander_set_expanded(
			GTK_EXPANDER(lookup_widget(win_prefs, "proprietary_formats_expander")), TRUE);
	}
...
void get_prefs_from_widgets(struct prefs *p)
{
	...
	p->make_playlist =
		gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "make_playlist")));
	p->format_music =
		strdup(gtk_entry_get_text(GTK_ENTRY(lookup_widget(win_prefs, "format_music"))));
	p->format_playlist =
		strdup(gtk_entry_get_text(GTK_ENTRY(lookup_widget(win_prefs, "format_playlist"))));
	p->format_albumdir =
		strdup(gtk_entry_get_text(GTK_ENTRY(lookup_widget(win_prefs, "format_albumdir"))));
	p->rip_wav =
		gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_wav")));
	p->do_fast_rip =
		gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "do_fast_rip")));
	p->rip_mp3 =
		gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_mp3")));
	p->rip_ogg =
		gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_ogg")));
	p->rip_flac =
		gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_flac")));
	p->rip_wavpack =
		gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_wavpack")));
	p->mp3_vbr =
		gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "mp3_vbr")));
	...
	p->flac_compression =
		(int)gtk_range_get_value(GTK_RANGE(lookup_widget(win_prefs, "flac_compression")));
	p->wavpack_compression =
		(int)gtk_range_get_value(GTK_RANGE(lookup_widget(win_prefs, "wavpack_compression")));
	p->wavpack_hybrid =
		gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "wavpack_hybrid")));
	p->wavpack_bitrate =
		(int)gtk_range_get_value(GTK_RANGE(lookup_widget(win_prefs, "wavpack_bitrate")));
	p->rip_monkey =
		gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_monkey")));
	p->monkey_compression =
		(int)gtk_range_get_value(GTK_RANGE(lookup_widget(win_prefs, "monkey_compression")));
	p->rip_aac =
	...
	p->rip_musepack =
		gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_musepack")));
	p->musepack_bitrate =
		(int)gtk_range_get_value(GTK_RANGE(lookup_widget(win_prefs, "musepack_bitrate")));
	p->rip_opus =
		gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "rip_opus")));
	p->opus_bitrate = (int)gtk_range_get_value(GTK_RANGE(lookup_widget(win_prefs, "opus_bitrate")));
	p->eject_on_done =
		gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "eject_on_done")));
	p->do_cddb_updates = gtk_toggle_button_get_active(
		GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "do_cddb_updates")));
	p->use_proxy =
		gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "use_proxy")));
	...
	p->cddb_server_name =
		strdup(gtk_entry_get_text(GTK_ENTRY(lookup_widget(win_prefs, "cddb_server_name"))));
	p->cddb_port_number =
		atoi(gtk_entry_get_text(GTK_ENTRY(lookup_widget(win_prefs, "cddb_port_number"))));
	p->do_log = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(win_prefs, "do_log")));
	p->more_formats_expanded =
		gtk_expander_get_expanded(GTK_EXPANDER(lookup_widget(win_prefs, "more_formats_expander")));
	p->proprietary_formats_expanded = gtk_expander_get_expanded(
		GTK_EXPANDER(lookup_widget(win_prefs, "proprietary_formats_expander")));
...
int prefs_are_valid(void)
{
	...
	// playlistfile
	if (strchr(gtk_entry_get_text(GTK_ENTRY(lookup_widget(win_prefs, "format_playlist"))), '/') !=
		NULL) {
	...
	// musicfile
	if (strchr(gtk_entry_get_text(GTK_ENTRY(lookup_widget(win_prefs, "format_music"))), '/') !=
		NULL) {
		warningDialog = gtk_message_dialog_new(GTK_WINDOW(win_main), GTK_DIALOG_DESTROY_WITH_PARENT,
											   GTK_MESSAGE_ERROR, GTK_BUTTONS_OK,
											   _("Invalid characters in the '%s' field"),
	...
	if (!strlen(gtk_entry_get_text(GTK_ENTRY(lookup_widget(win_prefs, "format_music"))))) {
		warningDialog = gtk_message_dialog_new(GTK_WINDOW(win_main), GTK_DIALOG_DESTROY_WITH_PARENT,
											   GTK_MESSAGE_ERROR, GTK_BUTTONS_OK,
											   _("'%s' cannot be empty"), _("Music file: "));
		gtk_dialog_run(GTK_DIALOG(warningDialog));
		gtk_widget_destroy(warningDialog);
		somethingWrong = 1;
	}
