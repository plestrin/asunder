...
void dorip(void){
    ...
    if (global_prefs->make_playlist){
        ...
        playlist = parse_format(global_prefs->format_playlist, 0, album_meta->year, album_meta->artist, album_meta->title, album_meta->genre, NULL);
        if (playlist != NULL){
            #define make_specific_playlist(playlist, ptr, str)                                                                                  \
                {                                                                                                                               \
                    char* filename;                                                                                                             \
                                                                                                                                                \
                    if ((filename = make_filename(prefs_get_music_dir(global_prefs), album_meta->directory, (playlist), (str))) == NULL){        \
                        fprintf(stderr, "unable to make filename\n");                                                                           \
                    }                                                                                                                           \
                    else{                                                                                                                       \
            ...
            if (global_prefs->rip_wav){
                make_specific_playlist(playlist, playlist_wav, "wav.m3u")
            }
            if (global_prefs->rip_mp3){
                make_specific_playlist(playlist, playlist_mp3, "mp3.m3u")
            }
            if (global_prefs->rip_ogg){
                make_specific_playlist(playlist, playlist_ogg, "ogg.m3u")
            }
            if (global_prefs->rip_opus){
                make_specific_playlist(playlist, playlist_opus, "opus.m3u")
            }
            if (global_prefs->rip_flac){
                make_specific_playlist(playlist, playlist_flac, "flac.m3u")
            }
            if (global_prefs->rip_wavpack){
                make_specific_playlist(playlist, playlist_wavpack, "wv.m3u")
            }
            if (global_prefs->rip_monkey){
                make_specific_playlist(playlist, playlist_monkey, "ape.m3u")
            }
            if (global_prefs->rip_musepack){
                make_specific_playlist(playlist, playlist_musepack, "mpc.m3u")
            }
            if (global_prefs->rip_aac){
                make_specific_playlist(playlist, playlist_aac, "m4a.m3u")
            }
